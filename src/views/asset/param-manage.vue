      ...
      <div class="table_wrap">
        ...
        <div class="table_con">
          <el-table :data="fenyelist" stripe style="width: 100%">
            <el-table-column prop="propertyCN" label="参数名称" width="150">
            </el-table-column>
            <el-table-column prop="propertyCode" label="参数标识" width="120">
            </el-table-column>
            <el-table-column prop="propertyType" label="参数类型" width="60">
            </el-table-column>
            <el-table-column prop="valueType" label="值类型">
            </el-table-column>
            <el-table-column prop="isAlarm" label="是否报警" width="60">
            </el-table-column>
            <el-table-column prop="alarmNumber" label="告警值">
            </el-table-column>
            <el-table-column prop="alarmLevel" label="告警等级" width="100">
            </el-table-column>
            <el-table-column prop="propertyUnit" label="参数单位">
            </el-table-column>
            <el-table-column prop="maxValue" label="数值上限" width="60">
            </el-table-column>
            <el-table-column prop="minValue" label="数值下限" width="60">
            </el-table-column>
...
import { getDevicePropertyListByTypeID } from '@/api/asset.js'
export default {
  name: 'ParamManage',
  data() {
    return {
  ...
  mounted (){
    let _this = this;
    _this.$nextTick( ()=> {
      getDevicePropertyListByTypeID(store.getters.userid).then( res => {
        // console.log(res);
        // return;
        if(res.success){
          ...
          res.result.forEach(val => {
            _this.usedata.push({
              propertyCN: val.propertyCN,
              propertyCode: val.propertyCode,
              propertyType: _this.dealpropertyType(val.propertyType),
              valueType: _this.dealvalueType(val.valueType),
              isAlarm:  _this.dealisAlarm(val.isAlarm),
              alarmNumber: _this.dealalarmNumber(val.alarmNumber),
              alarmLevel: _this.dealalarmLevel(val.alarmLevel),
