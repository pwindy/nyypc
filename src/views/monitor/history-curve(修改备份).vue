      ...
      <div class="chose_wrap">
        ...
        <div v-if="valuetype === '0'" class="selecttime_wrap">
          ...
          <div class="select_con">
            ...
            <el-date-picker v-model="valuedetail" type="datetimerange" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"> </el-date-picker>
          </div>
        ...
        <div v-else-if="valuetype === '1'" class="selecttime_wrap">
          <div class="select_title">时间选择：</div>
          <div class="select_con">
            <el-date-picker v-model="valuedetail" type="datetimerange" format="yyyy-MM-dd HH" value-format="yyyy-MM-dd HH" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"> </el-date-picker>
          </div>
          <el-button class="checkdata" @click="subdata" type="primary" :loading="loadingbtn">{{loadingbuttext}}</el-button>
        </div>
        ...
        <div v-else-if="valuetype === '2'" class="selecttime_wrap">
          <div class="select_title">时间选择：</div>
          <div class="select_con">
            <el-date-picker v-model="valuedetail" type="daterange" format="yyyy-MM-dd" value-format="yyyy-MM-dd" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"> </el-date-picker>
          </div>
          <el-button class="checkdata" @click="subdata" type="primary" :loading="loadingbtn">{{loadingbuttext}}</el-button>
        </div>
        ...
        <div v-else-if="valuetype === '3'" class="selecttime_wrap">
          <div class="select_title">时间选择：</div>
          <div class="select_con">
            <el-date-picker v-model="valuedetail" type="monthrange" format="yyyy-MM" value-format="yyyy-MM" range-separator="至" start-placeholder="开始月份" end-placeholder="结束月份"> </el-date-picker>
          </div>
          <el-button class="checkdata" @click="subdata" type="primary" :loading="loadingbtn">{{loadingbuttext}}</el-button>
        </div>
      </div>
    ...
    export default {
      ...
      methods:{
        ...
        async subdata(){
          ...
          if(_this.trueChecked.length>0 && _this.valuetype !='' && _this.valuedetail != ''){
            _this.loadingbtn = true;
            _this.loadingbuttext = '查询中...';
            if(_this.chart_curve_wrap._chartsViews.length > 0){
            ...
            if( _this.valuetype == 1 ){
              _this.valuedetail[0] = _this.valuedetail[0] + ':00';
              _this.valuedetail[1] = _this.valuedetail[1] + ':00';
            }


            for (var element of _this.trueChecked) {
              // 提供的查询数据
              var postdata = {
                devid: element.devid,
                typeid: element.typeid,
                timetype: _this.valuetype,
                columnName: element.columnName,
                start: new Date(_this.valuedetail[0]).valueOf(), // 时间范围的开始时间(new Date()).valueOf();
                end: new Date(_this.valuedetail[1]).valueOf(), // 时间范围的结束时间
              };
              ...
              const userdata = await getHistoryData(postdata);
              if(userdata.success){
                ...
                for (var i = 0; i < userdata.result.length; i++) {
                  ...
                  let time;
                  if (_this.valuetype == 0) {
                    // 分
                    time = getMonth + '-' + getDate + ' ' + getHours + ':' + getMinutes;
                  } else if (_this.valuetype == 1) {
                    // 时
                    time = getMonth + '-' + getDate + ' ' + getHours + ':' + getMinutes;
                  } else if (_this.valuetype == 2) {
                    // 日
                    time = getMonth + '-' + getDate;
                  } else if (_this.valuetype == 3) {
                    // 月
                ...
                _this.xAxis.push(xdatatimeaa)

                var seriesdata = {};
                seriesdata.name = element.parenttitle + element.title;
                seriesdata.type = 'line';
                seriesdata.data = ydataaa;
                _this.seriesarr.push(seriesdata);
                _this.legendarr.push(element.parenttitle + element.title);
              }
            }

            if (_this.legendarr.length == _this.trueChecked.length) {

              var arry = JSON.parse(JSON.stringify(_this.seriesarr));
              var arrx = JSON.parse(JSON.stringify(_this.xAxis));


              var maxindex = _this.getMaxIndex(arrx);

              var getitemindex = _this.getitemindex(arrx,maxindex);

              for(var m in arry){
                for(var n=0;n < getitemindex[m].length; n++){
                  arry[m].data.splice(getitemindex[m][n], 0, '/');
                }
              }

              var ll = _this.legendarr;
              _this.chartData = {
                tooltip: {
                  trigger: 'axis',
              ...
              this.chart_curve_wrap.setOption(_this.chartData);
        ...
        getitemindex(arrx,maxindex){
          var indexarr = [];

          for(var i=0;i<arrx.length; i++){
            var newarr= [];
            for(var j=0;j<arrx[i].length; j++ ){
              if( arrx[i][j] != arrx[maxindex][j]){
                // console.log('不一样');
                arrx[i].splice(j,0,'00')
                newarr.push(Number(j));
              }
            }

            if(arrx[i].length < arrx[maxindex].length){
              for(var k=arrx[i].length;k<arrx[maxindex].length; k++ ){
                arrx[i].splice(k,0,'00')
                newarr.push(k);
              }
            }
