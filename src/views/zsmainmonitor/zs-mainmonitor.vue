...
export default {
  ...
  methods:{
    ...
    getfirstqx(){
        getListControlDataByID('17').then(data => {
          let szdata = [];

          for (var i in data.result) {
            if (data.result[i].devID === 17) {
        ...
        if(res.success == true){
          ...
          if(this.num > 1){
            if(data[0].time){
              ...
              this.yData_fd.push( data[4].farTAP );// 电网功率
              this.yData_pcs.push( data[4].pcstap );// PCS功率
              this.yData_fz.push( data[4].loadTAP ); // 负载功率
